...
const controlRecipes = async function() {
    try {
        ...
        // 2. Loading recepies 
        await _modelJs.loadRecipe(id) // async function that return a promise
        ;
        // 3. Display recipe, servings and the bookmarks
        (0, _recipeViewJs.recipeView).render(_modelJs.state.recipe);
        // 4. Reset the servings
        (0, _recipeViewJs.recipeView).servings = 4;
        // 5. Update the recipe
        (0, _recipeViewJs.recipeView).update(_modelJs.state.recipe);
        // 6. Update the results and the bookmark for the active recipe
        if (_modelJs.state.bookmarks.length !== 0) (0, _bookmarksViewJs.bookmarksView).update(_modelJs.state.bookmarks);
        if (_modelJs.state.search.results.length !== 0) (0, _resultsViewJs.resultsView).update((0, _resultsViewJs.resultsView).paginatedItems);
    } catch (err) {
        (0, _recipeViewJs.recipeView).renderError(err.message);
...
const controlResults = async function(e) {
    try {
        ...
        (0, _paginationViewJs.paginationView).clear();
        // 2. Render the spinner
        (0, _resultsViewJs.resultsView).renderSpinner();
        // 3. Catch the input value
        const searched = (0, _searchViewJs.searchView).getQuery();
        if (!searched) throw new Error("Please write some recive");
        // 4. Pass it into model.load()
        await _modelJs.loadResults(searched);
        // 5. Set the current page at 1
        _modelJs.state.search.currentPage = 1;
        (0, _paginationViewJs.paginationView).currentPage = _modelJs.state.search.currentPage;
        // 6. Render 10 results per page
        const resultsPerPage = showPage(_modelJs.state.search.currentPage, _modelJs.state.search.results);
        (0, _resultsViewJs.resultsView).render(resultsPerPage);
        ...
        (0, _searchViewJs.searchView).clearInput();
    }
};
const showPage = function(page, items) {
    ...
    const start = (page - 1) * (0, _configJs.ITEM_PER_PAGE);
    const end = page * (0, _configJs.ITEM_PER_PAGE);
    ...
    (0, _paginationViewJs.paginationView).render(true);
    // 5. Define the condition to control the buttons
    (0, _paginationViewJs.paginationView).viewBtn((0, _paginationViewJs.paginationView).currentPage === _modelJs.state.search.totalPage, _modelJs.state.search.currentPage === 1);
    (0, _resultsViewJs.resultsView).paginatedItems = paginatedItems;
...
const controlPage = function(e) {
    ...
    if (!(_helpersJs.getTotalPages(_modelJs.state.search.results) > _modelJs.state.search.currentPage || _modelJs.state.search.currentPage > 1)) return;
    // 2. Update the number of the page and the current page
    const { goTo } = e.target.closest("button").dataset;
    _modelJs.state.search.currentPage = +goTo;
    (0, _paginationViewJs.paginationView).currentPage = _modelJs.state.search.currentPage;
...
const setNewQuantity = function(recipe, newServings) {
    recipe.ingredients.forEach((ing)=>ing.quantity = ing?.quantity * newServings / (0, _recipeViewJs.recipeView).servings);
};
const controlServings = function(e, newServings) {
    // 1. Guard: previus servings = 0
    if (newServings === 0) return;
    // 2. Increment or decrement the servings
    setNewQuantity(_modelJs.state.recipe, newServings);
    // 3. Set the new servings into the VIEW
    (0, _recipeViewJs.recipeView).servings = newServings;
...
const controlBookmarks = function() {
    ...
    const indexBookmark = _modelJs.state.bookmarks?.findIndex((bookmark)=>bookmark.id === recipe.id);
    ...
    if (indexBookmark === -1) {
        ...
        _modelJs.state.bookmarks.push(recipe);
    }
    ...
    if (indexBookmark !== -1) {
        ...
        _modelJs.state.bookmarks.splice(indexBookmark, 1);
    }
    // 3. Update bookmarks in the model
    _modelJs.setBookmark();
    // 4. Render the bookmarks
    (0, _bookmarksViewJs.bookmarksView).render(_modelJs.state.bookmarks);
    // 5. Update the bookmarks into the VIEW 
    (0, _recipeViewJs.recipeView).update(_modelJs.state.recipe);
    (0, _bookmarksViewJs.bookmarksView).update(_modelJs.state.bookmarks);
};
...
const controlAddRecipe = async function(newRecipe) {
    try {
        // 1.1 Render Spinner
        (0, _addRecipeViewJs.addRecipeView).renderSpinner();
        // 1. Creating the object
        await _modelJs.uploadRecipe(newRecipe);
        // 2. Render the success message
        (0, _addRecipeViewJs.addRecipeView).renderError((0, _addRecipeViewJs.addRecipeView).message, "message");
        ...
        setTimeout(()=>{
            if ((0, _addRecipeViewJs.addRecipeView).checkCloseWindow()) return;
            (0, _addRecipeViewJs.addRecipeView).controlClassList();
        ...
        (0, _addRecipeViewJs.addRecipeView).renderError(err.message);
        console.error(err);
    }
};
...
const init = function() {
    ...
    (0, _recipeViewJs.recipeView).addHandlerServings(controlServings);
    // Onsubmit calc and render the results and the pagination
    (0, _searchViewJs.searchView).addHandlerSearch(controlResults);
    (0, _paginationViewJs.paginationView).addHandlerPagination(controlPage);
    // Onload render the bookmarks from the localstorage
    (0, _bookmarksViewJs.bookmarksView).addHandlerLoad(loadBookmarks);
    // On click render the bookmarks
    (0, _bookmarksViewJs.bookmarksView).addHandlerBookmarks(controlBookmarks);
    (0, _addRecipeViewJs.addRecipeView).addHandlerUpload(controlAddRecipe);
    newFeature();
};
init();

},{"core-js/modules/web.immediate.js":"49tUX","regenerator-runtime/runtime":"dXNgZ","./model.js":"Y4A21","./views/recipeView.js":"l60JC","./views/searchView.js":"9OQAM","./views/resultsView.js":"cSbZE","./views/paginationView.js":"6z7bi","./views/bookmarksView.js":"4Lqzq","./views/addRecipeView.js":"i6DNj","./config.js":"k5Hzs","./helpers.js":"hGI1E"}],"49tUX":[function(require,module,exports) {
"use strict";
...
parcelHelpers.export(exports, "setBookmark", ()=>setBookmark);
...
const loadRecipe = async function(id) {
    try {
        // 1. Fetch the recipe
        const data = await _helpers.controlJSON(`${(0, _config.API_URL)}${id}?key=${(0, _config.KEY)}`);
        console.log(data);
        // 2. Set the recipe in the state
        state.recipe = _helpers.formatData(data.data.recipe);
...
const loadResults = async function(query) {
    try {
        ...
        const data = await _helpers.controlJSON(`${(0, _config.API_URL)}?search=${query}&key=${(0, _config.KEY)}`);
        ...
        state.search.results = data.data.recipes.map((result)=>_helpers.formatResultSearch(result));
        // 5. Calc the total page for render the results
        state.search.totalPage = _helpers.getTotalPages(state.search.results);
    } catch (err) {
        throw err;
    }
};
...
const uploadRecipe = async function(newRecipe) {
    try {
        ...
        // 3. Send the request 
        const request = await _helpers.controlJSON(`${(0, _config.API_URL)}?key=${(0, _config.KEY)}`, recipe);
        // 4. Set the new recipe with the correct format
        state.recipe = _helpers.formatData(request.data.recipe);
...
//Put all the variables that we reuse during the project
//USE UPPER CASE FOR THIS TYPE OF COSTANT
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const KEY = "23130ed9-bd72-4bf4-acee-f50b3c967a3b";
const MODAL_CLOSE_SEC = 2.5 // export const SERVINGS = 4
;
...
parcelHelpers.export(exports, "controlJSON", ()=>controlJSON);
parcelHelpers.export(exports, "formatData", ()=>formatData);
parcelHelpers.export(exports, "formatResultSearch", ()=>formatResultSearch);
...
const controlJSON = async function(url, uploadRecipe) {
    try {
...
const getTotalPages = function(items) {
    const totalPages = Math.ceil(items.length / (0, _configJs.ITEM_PER_PAGE));
    (0, _recipeViewJs.recipeView).totalPage = totalPages;
    return totalPages;
};

},{"./config.js":"k5Hzs","./views/recipeView.js":"l60JC","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"l60JC":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class recipeViewClass extends (0, _viewDefault.default) {
    ...
    _generateMarkup() {
        ...
        const markup = `
          ...
          <button class="btn--round bookmark-button">
            <svg class="">
              <use href="${(0, _iconsSvgDefault.default)}#icon-bookmark${this._data.isBookmarked ? "-fill" : ""}" class="bookmark"></use>
            </svg>
          </button>
          </div>
          ...
          <div class="recipe__ingredients">
            ...
            <li class="recipe__ingredient">
              ...
              <div class="recipe__quantity">${cur.quantity !== null ? (0, _fractyDefault.default)(cur.quantity) : ""}</div>
...
const recipeView = new recipeViewClass();
...
var _iconsSvg = require("../../img/icons.svg"); //Parcel 2
var _iconsSvgDefault = parcelHelpers.interopDefault(_iconsSvg);
class searchViewClass extends (0, _viewJsDefault.default) {
...
const searchView = new searchViewClass();

},{"./View.js":"5cUXS","../../img/icons.svg":"cMpiy","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"cSbZE":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
var _iconsSvg = require("../../img/icons.svg"); //Parcel 2
var _iconsSvgDefault = parcelHelpers.interopDefault(_iconsSvg);
class resultsViewClass extends (0, _viewJsDefault.default) {
...
const resultsView = new resultsViewClass();
...
var _iconsSvg = require("../../img/icons.svg"); //Parcel 2
var _iconsSvgDefault = parcelHelpers.interopDefault(_iconsSvg);
class previewViewClass extends (0, _viewJsDefault.default) {
    _data;
    generateMarkup(data) {
...
exports.default = PreviewView = new previewViewClass();
...
var _iconsSvg = require("../../img/icons.svg"); //Parcel 2
var _iconsSvgDefault = parcelHelpers.interopDefault(_iconsSvg);
class paginationViewClass extends (0, _viewJsDefault.default) {
...
const paginationView = new paginationViewClass();

},{"regenerator-runtime":"dXNgZ","./View.js":"5cUXS","../../img/icons.svg":"cMpiy","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"4Lqzq":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
var _viewJs = require("./View.js");
var _viewJsDefault = parcelHelpers.interopDefault(_viewJs);
class bookmarksViewClass extends (0, _viewJsDefault.default) {
...
const bookmarksView = new bookmarksViewClass();

},{"./previewView.js":"1FDQ6","../../img/icons.svg":"cMpiy","./View.js":"5cUXS","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"i6DNj":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "addRecipeView", ()=>addRecipeView);
var _view = require("./View");
var _viewDefault = parcelHelpers.interopDefault(_view);
class addRecipeViewClass extends (0, _viewDefault.default) {
...
const addRecipeView = new addRecipeViewClass();

},{"./View":"5cUXS","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["hycaY","aenu9"], "aenu9", "parcelRequire807d")
